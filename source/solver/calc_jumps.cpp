...
namespace NSFEMSolver
{
  ...
  template <int dim>
  double NSolver<dim>::calc_jumps (const FEFaceValuesBase<dim>                          &fe_v_face_this,
                                   ...
                                   const bool                                            accumulate_value_jump)
  {
    ...
    if (accumulate_grad_jump)
      {
        ...
        for (unsigned int q=0; q<n_q_points; ++q)
          {
            const Tensor<1,dim> normal_vector = fe_v_face_this.normal_vector (q);
            for (unsigned int c=0; c<EquationComponents<dim>::n_components; ++c)
              {
                max_grad_jump[c] = std::max (max_grad_jump[c],
    ...
    if (accumulate_value_jump)
      {
        const bool is_on_boundary = (boundary_id != numbers::invalid_unsigned_int);
        ...
        for (unsigned int q=0; q<n_q_points; ++q)
          {
            ...
            for (unsigned int c=0; c<EquationComponents<dim>::n_components; ++c)
              {
                max_value_jump[c] = std::max (max_value_jump[c],

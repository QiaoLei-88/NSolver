...
namespace NSFEMSolver
{
  ...
  template <int dim>
  void
  NSolver<dim>::check_negative_density_pressure() const
  {
          ...
          for (unsigned int q = 0; q < n_q_points; ++q)
            {
              const double density =
                solution_values[q](EquationComponents<dim>::density_component);
              AssertThrow(density > 0.0,
                          ExcMessage("Negative density encountered!"));
              const double pressure =
                solution_values[q](EquationComponents<dim>::pressure_component);
              AssertThrow(pressure > 0.0,
                          ExcMessage("Negative pressure encountered!"));
            }
        }

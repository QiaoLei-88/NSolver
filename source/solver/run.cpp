...
namespace NSFEMSolver
{
  ...
  NSolver<dim>::run()
  {
    computing_timer.enter_subsection("0:Read grid");
    {
      if (parameters->n_mms == 1)
      ...
      else
        {
          ...
          if (parameters->mesh_format ==
              Parameters::AllParameters<dim>::format_ucd)
            {
              grid_in.read_ucd(input_file);
            }
          if (parameters->mesh_format ==
              Parameters::AllParameters<dim>::format_gmsh)
            {
      ...
      if (parameters->manifold_circle > 0)
        {
          ...
          Assert(parameters->NACA_foil == 0,
                 ExcMessage(
                   "NACA_foil and C1Circle case can't play together!!!"));
          ...
          if (parameters->manifold_circle == 1)
            {
              // Circle
              spherical_boundary = new SphericalManifold<dim>(
          ...
          else if (parameters->manifold_circle == 2)
            {
              // GAMM Channel
              spherical_boundary =
                new SphericalManifold<dim>(Point<dim>(0.0, -1.2));
            }
          triangulation.set_manifold(2, *spherical_boundary);

          if (parameters->manifold_circle == 2)
            {
              // Borrow a parameter. This is not good. And I know its not good.
              // Only this one time.
                  ...
                  for (int n = 0; n < 1; ++n)
                    {
                      for (typename Triangulation<dim>::active_cell_iterator
                           ...
                           ++cell)
                        {
                          ...
                          const double a =
                            (cell->center()[0] - center_x) / half_axi_long;
                          const double b =
                            (cell->center()[1] - center_y) / half_axi_short;
                          if ((a * a + b * b) < 1.0)
                            {
                       ...
                       ++cell)
                    {
                      ...
                      if (parameters->NACA_cheating_refinement == 2)
                        if (cell->center()[0] > 0.15 &&
                            cell->center()[0] < 0.55 && cell->center()[1] < 1.0)
                          {
                            cell->set_refine_flag();
                          }
                    }
    ...
    parameters_modifier->count_solution_diff_in_residual =
      parameters->count_solution_diff_in_residual || (!parameters->is_steady);
    ...
    const double linear_search_length[12] = {
      1.0, 0.8, 0.6, 0.4, 0.2, 0.1, 0.05, 0.025, 0.0125, 1.2, 1.5, 2.0};
    unsigned int converged_newton_iters(0);

    n_time_step = 0;
    const unsigned int int_output_step =
      static_cast<unsigned int>(parameters->output_step);
    ...
    while (!terminate_time_stepping)
      {
        ...
        pcout
          << "   NonLin Res   NewtonUpdateNorm  Lin Iter     Lin Res     "
          << "Linear Search Len      Time Step Size      CFL number   ||Res||_infty ||Res||_infty_all"
          << std::endl
        ...
        // Set up continuation coefficients for pseudo time component and
        // laplacian component
        pcout << "continuation_coefficient = " << continuation_coefficient
              << std::endl;
        switch (parameters->continuation_type)
          {
            case Parameters::StabilizationParameters<dim>::CT_timeCFL:
              {
                AssertThrow(
                  ...
                  ExcMessage(
                    "Laplacian continuation should be disabled in timeCFL mode!"));
            ...
            case Parameters::StabilizationParameters<dim>::CT_timeCFL2:
              {
                AssertThrow(CFL_number == 1.0,
                            ExcMessage(
                              "CFL number should be 1.0 in timeCFL2 mode!"));
                // Works are done by calc_time_step().
                // Nothing needs to do here.
                break;
              }
            ...
            case Parameters::StabilizationParameters<dim>::CT_switch:
              {
                if (continuation_coefficient >
                    mean_artificial_viscosity *
                      parameters->continuation_switch_threshold)
                  {
                    continuation_coeff_time      = 0.0;
                    continuation_coeff_laplacian = continuation_coefficient;
                  }
            ...
            case Parameters::StabilizationParameters<dim>::CT_blend:
              {
                if (n_time_step == 1)
                  {
                    continuation_mu_blend_weight =
                      ...
                      (parameters->continuation_blend_starting_ratio *
                       mean_artificial_viscosity);
                  }
                const double weight =
                  continuation_coefficient /
                ...
                continuation_coeff_time =
                  (1.0 - weight) * continuation_coefficient;
                continuation_coeff_laplacian =
                  weight * continuation_coefficient;
                break;
              }
            default:
              {
        ...
        for (bool terminate_newton_iteration = false;
             !terminate_newton_iteration;)
          {
            paper_data_out << std::setw(10) << n_total_iter + 1 << ' '
            ...
            if (parameters->output_system_matrix)
              {
                const std::string file_name =
                  "system_matrix.step_" +
                  Utilities::int_to_string(n_time_step, 4) + ".iter_" +
                  Utilities::int_to_string(nonlin_iter, 4) + ".MTX";
            ...
            paper_data_out << std::setw(13) << res_norm << ' ' << std::setw(13)
                           << physical_res_norm << ' ';
            ...
            if (nonlin_iter == 0)
              {
                ...
                terminal_res =
                  parameters->nonlinear_tolerance >= 0.0 ?
            ...
            physical_residual_ratio =
              physical_res_norm / physical_residual_first;
            ...
            Assert(index_linear_search_length < 9,
                   ExcIndexRange(index_linear_search_length, 0, 9));
            ...
            if (I_am_host)
              {
                std::printf(
                  "   %-13.6e    %-13.6e  %04d        %-5.2e            %7.4g          %7.4g          %7.4g      %11.4e    %11.4e    %11.4e\n",
                  res_norm,
                  newton_update_norm,
                  convergence.first,
                  convergence.second,
            ...
            iteration_history_file
              << std::setw(7) << n_total_iter << ' ' << std::setw(10)
              << triangulation.n_global_active_cells() << ' ' << std::setw(10)
              << dof_handler.n_dofs() << ' ' << std::setw(13) << time << ' '
              << std::setw(8) << n_time_step << ' ' << std::setw(9)
              << nonlin_iter << ' ' << std::setw(13) << res_norm << ' '
              << std::setw(14) << convergence.first << ' ' << std::setw(13)
              << convergence.second << ' ' << std::setw(18)
              << linear_search_length[index_linear_search_length] << ' '
              << std::setw(15) << global_time_step_size << ' ' << std::setw(13)
              << CFL_number << ' ' << std::setw(19) << newton_update_norm
              << '\n';
            // Check result.
            ...
            // Using '>=' here because this condition is evaluated after
            // '++nonlin_iter'.
            if (nonlin_iter >= nonlin_iter_threshold)
              {
                ...
                if (!newton_iter_converged)
                  {
                    pcout << "  Newton iteration not converge in "
                          << nonlin_iter_threshold << " steps.\n";
                  }
              }

            if (parameters->is_steady && newton_iter_converged &&
                (parameters->continuation_type ==
                   Parameters::StabilizationParameters<dim>::CT_timeCFL ||
                 parameters->continuation_type ==
                   Parameters::StabilizationParameters<dim>::CT_timeCFL2) &&
                // Physical residual and algebra residual are close to each
                // other
                std::abs(res_norm - physical_res_norm) <=
                  0.1 * physical_res_norm &&
                // Solution is not converged to demanded final tolerance
                terminal_res > parameters->physical_residual_l2_tolerance)
              {
                // This means the local problem is close enough to the real
                // problem, we should keep doing the Newton iteration to the
                // demanded final tolerance.
        ...
        if (newton_iter_converged)
          {
            ...
            pcout << "  Lift and drag:" << std::endl
                  << "  " << wall_force.lift << "  " << wall_force.drag
                  << std::endl
                  << std::endl;
            ...
            pcout << std::endl << "  Moment,x,y,z:" << std::endl << "  ";
            for (unsigned int id = 0; id < 3; ++id)
              {
            ...
            ++n_time_step;
            terminate_time_stepping =
              terminate_time_stepping || time >= parameters->final_time;
            terminate_time_stepping =
              terminate_time_stepping ||
              (parameters->is_steady && n_time_step > parameters->final_time);

            /********************** Determine mesh adaption
             * ********************/
            bool do_refine =
              ((parameters->is_steady) ?
                 (static_cast<unsigned int>(parameters->max_refine_time) >=
                  n_time_step) :
                 (parameters->max_refine_time >= time));
            do_refine = do_refine && (!(parameters->dodge_mesh_adaptation) ||
                                      is_refine_step);
            if (parameters->laplacian_continuation > 0.0)
              {
                do_refine =
                  do_refine && n_time_step <= n_step_laplacian_vanished +
                                                parameters->max_refine_level;
              }
            ...
            if (parameters->is_steady)
                ...
                if (!(parameters->dodge_mesh_adaptation) || is_refine_step)
                  {
                        ...
                        if (parameters->step_with_physical_residual)
                          {
                            CFL_number = std::max(
                              CFL_number *
                                parameters
                                  ->minimum_step_increasing_ratio_stage2,
                              parameters->CFL_number *
                                std::pow(
                                  1.0 / physical_residual_ratio,
                                  parameters->step_increasing_power_stage2));
                          }
                        else
                          {
                            double const ratio = std::max(
                              parameters->minimum_step_increasing_ratio_stage2,
                              std::pow(
                                res_norm_total_previous / res_norm_total,
                                parameters->step_increasing_power_stage2));
                            CFL_number *= ratio;
                    ...
                    CFL_number =
                      std::min(CFL_number, parameters->CFL_number_max);
              ...
              {
                // Since every thing goes so well, let's try a larger time step
                // next.
                CFL_number *= 2.0;
            ...
            // MMS: update quadrature points for evaluation of manufactured
            // solution.
            const UpdateFlags update_flags = update_values | update_gradients |
                                             update_JxW_values |
                                             update_quadrature_points;
            const QGauss<dim> quadrature_error(
              parameters->error_quadrature_degree);
            FEValues<dim> fe_v(*mapping_ptr,
                               fe,
                               quadrature_error,
                               update_flags);
              ...
              if (cell->is_locally_owned())
                {
                  ...
                  solution_grad.resize(
                    n_q_points,
                    std::vector<Tensor<1, dim>>(
                      EquationComponents<dim>::n_components));
                  old_solution_values.resize(
                    n_q_points,
                    Vector<double>(EquationComponents<dim>::n_components));
                  ...
                  for (unsigned int q = 0; q < n_q_points; ++q)
                    {
                      local_Mach_max = std::max(
                        local_Mach_max,
                        EulerEquations<dim>::compute_velocity_magnitude(
                          solution_values[q]) /
                          EulerEquations<dim>::compute_sound_speed(
                            solution_values[q]));
                    }
                  ...
                  for (unsigned int ic = 0;
                       ...
                       ++ic)
                    {
                      for (unsigned int q = 0; q < n_q_points; ++q)
                        {
                          time_advance_l2_norm[ic] +=
                            (old_solution_values[q][ic] -
                             solution_values[q][ic]) *
                            (old_solution_values[q][ic] -
                             solution_values[q][ic]) *
                            fe_v.JxW(q);
                        }
                          ...
                          for (unsigned int q = 0; q < n_q_points; ++q)
                            {
                              ...
                              for (unsigned int d = 0; d < dim; ++d)
                                {
                                  grad_diff += (mms_grad[q][ic][d] -
                                                solution_grad[q][ic][d]) *
                                               (mms_grad[q][ic][d] -
                                                solution_grad[q][ic][d]);
                                }
            ...
            global_Mach_max =
              Utilities::MPI::max(local_Mach_max, mpi_communicator);

            if (parameters->n_mms == 1)
              {
                pcout << "  Error Info:\n";
                pcout
                  << "    n_dofs    u_err    v_err  rho_err    p_err (log10)\n   ";
                ...
                for (unsigned int ic = 0;
                  ...
                  {
                    const double received_sum =
                      Utilities::MPI::sum(mms_error_H1_semi[ic],
                                          mpi_communicator);
            ...
            // Never decease continuation_coefficient and do mesh refine
            // together.
            if (!(parameters->dodge_mesh_adaptation) || !is_refine_step)
              {
                old_continuation_coefficient = continuation_coefficient;
                {
                  ...
                  double laplacian_ratio_min =
                    parameters->continuation_min_decrease_rate;
                  ...
                  if (parameters->compute_laplacian_coeff_from_Mach_max)
                    {
                      pcout << "global_Mach_max = " << global_Mach_max
                            << std::endl;
                      const double effective_Mach_max =
                        std::min(1.0, global_Mach_max);
                      const double &Mach_infty = parameters->Mach;
                      referencing_continuation_coeff *= effective_Mach_max *
                                                        effective_Mach_max /
                                                        Mach_infty / Mach_infty;
                    }

                  continuation_coefficient = std::min(
                    laplacian_ratio_min * continuation_coefficient,
                    referencing_continuation_coeff *
                      std::pow(
                        physical_residual_ratio,
                        parameters->continuation_decrease_residual_power));

                  const double terminal_continuation_coefficient =
                    parameters->laplacian_zero > 0.0 ?
                      parameters->laplacian_zero :
                      std::pow(10.0, parameters->laplacian_zero) *
                        mean_artificial_viscosity;

                  if (continuation_coefficient <
                      terminal_continuation_coefficient)
                    {
            ...
            // types are different.
            bool swith_flux = parameters->numerical_flux_type !=
                              parameters->flux_type_switch_to;
            // If switch of flux type is requested and the flux switch tolerance
            // is larger than time march tolerance, never stop time marching
            // before switching to the target flux type.
            bool time_march_converged =
              !(swith_flux && parameters->tolerance_to_switch_flux >
                                parameters->solution_update_l2_tolerance);
            time_march_converged =
              time_march_converged && continuation_coefficient_vanished;
            ...
            for (unsigned int ic = 0;
              ...
              {
                double receive_sum;
                receive_sum = Utilities::MPI::sum(time_advance_l2_norm[ic],
                                                  mpi_communicator);
                time_advance_l2_norm[ic] = receive_sum;
                total_time_march_norm += time_advance_l2_norm[ic];
                double const log_norm =
                  0.5 * std::log10(time_advance_l2_norm[ic]);
            ...
            pcout << std::endl;
            time_advance_history_file
              << std::setw(15) << std::sqrt(total_time_march_norm) << '\n';
            if (swith_flux)
              {
                parameters_modifier->numerical_flux_type =
                  parameters->flux_type_switch_to;
              }
            ...
            if (parameters->physical_residual_l2_tolerance >= 0.0)
            ...
            else
              {
                time_march_converged =
                  time_march_converged &&
                  std::log10(physical_residual_ratio) <
                    parameters->physical_residual_l2_tolerance;
              }
            if (parameters->laplacian_continuation > 0.0)
              {
            ...
            terminate_time_stepping =
              terminate_time_stepping ||
              (parameters->is_steady && time_march_converged);
            ...
            check_negative_density_pressure();
            calc_time_step();
            // Uncomment the following line if you want reset the
            // linear_search_length immediately after a converged Newton iter.
            // index_linear_search_length = 0;
            ...
            if (parameters->laplacian_continuation > 0.0)
              {
                if (continuation_coefficient > 0.0)
                  {
                    continuation_coefficient = std::sqrt(
                      old_continuation_coefficient * continuation_coefficient);
                  }
                else
                  {
                    continuation_coefficient =
                      0.5 * old_continuation_coefficient;
                  }
                pcout << "reseted continuation_coefficient = "
                      << continuation_coefficient << std::endl;
              }
              ...
              {
                AssertThrow(
                  parameters->allow_decrease_CFL,
                  ExcMessage(
                    "Nonlinear not convergence and reducing CFL number is disabled."));
                // Reduce time step when linear_search_length has tried out.
                CFL_number *= 0.5;
                AssertThrow(
                  CFL_number >= parameters->CFL_number_min,
                  ExcMessage(
                    "No convergence in nonlinear solver after all small time step and linear search length tried out."));
        ...
        if (!(parameters->is_steady))
          {
            ...
            double const predict_ratio =
              parameters->solution_extrapolation_length *
    ...
    // Timer initialized with TimerOutput::summary will print summery
    // information on its destruction. computing_timer.print_summary();

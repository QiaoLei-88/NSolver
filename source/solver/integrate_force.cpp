...
namespace NSFEMSolver
{
  ...
  template <int dim>
  void
  NSolver<dim>::integrate_force(WallForce &wall_force) const
  {
    ...
    const std::string file_name =
      "boundaryData.slot-" + Utilities::int_to_string(myid, 4) + ".raw";
            ...
            if (parameters->sum_force[cell->face(face_no)->boundary_id()])
              {
                ...
                fe_v_face.get_function_values(current_solution,
                                              solution_values);

                for (unsigned int q = 0; q < n_q_points; ++q)
                  {
                    Tensor<1, dim> const wall_norm = fe_v_face.normal_vector(q);
                    Point<dim> const     wall_position =
                      fe_v_face.quadrature_point(q);
                    double f_ds[3];
                    ...
                    for (unsigned int id = 0; id < dim; ++id)
                      {
                        f_ds[id] =
                          solution_values
                            [q][EquationComponents<dim>::pressure_component] *
                          wall_norm[id] * fe_v_face.JxW(q);
                        wall_force.force[id] += f_ds[id];
                        moment_arm[id] =
                          wall_position[id] - parameters->moment_center[id];
    ...
    double const force_to_coeff =
      0.5 * parameters->Mach * parameters->Mach * parameters->reference_area;
    ...
    wall_force.lift =
      cos_aoa * wall_force.force[1] - sin_aoa * wall_force.force[0];
    wall_force.drag =
      cos_aoa * wall_force.force[0] + sin_aoa * wall_force.force[1];

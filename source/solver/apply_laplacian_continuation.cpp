...
namespace NSFEMSolver
{
  ...
  NSolver<dim>::apply_laplacian_continuation()
  {
      ...
      if (cell->is_locally_owned())
        {
          ...
          if ((n_time_step > 0) && parameters->enable_partial_laplacian)
            {
              if (laplacian_indicator[cell->active_cell_index()] <
                  laplacian_threshold)
                {
          ...
          if (parameters->use_local_laplacian_coefficient &&
              (local_laplacian_coeff > 0.0) && (n_time_step > 0))
            {
          ...
          if (parameters->continuation_type ==
          ...
          if (write_paper_data)
            {
              ...
              paper_data_out
                ...
                << ' ' << std::setw(13)
                << std::max(local_laplacian_coeff, 1.0e-99) << ' ';
              ...
              for (unsigned int i = 0; i < dofs_per_cell; ++i)
                {
                  ...
                  for (unsigned int q = 0; q < n_q_points; ++q)
                    {
                      residual +=
                        local_laplacian_coeff *
                        (grad_W[q][c] * fe_v.shape_grad_component(i, q, c)) *
                        fe_v.JxW(q);
                    }
          ...
          if (local_time_coeff > 0.0)
            {
              ...
              std::array<DFADD, EquationComponents<dim>::n_components>
                w_conservative;
                      ...
                      for (unsigned int q = 0; q < n_q_points; ++q)
                        {
                          EulerEquations<dim>::compute_conservative_vector(
                            W[q], w_conservative);
                          EulerEquations<dim>::compute_conservative_vector(
                            W_old[q], w_conservative_old);
                          residual +=
                            local_time_coeff *
                            (w_conservative[c] - w_conservative_old[c]) *
                            fe_v.shape_value_component(i, q, c) * fe_v.JxW(q);
                        }
                    }
                  else
                    {
                      for (unsigned int q = 0; q < n_q_points; ++q)
                        {
                          residual +=
                            local_time_coeff * (W[q][c] - W_old[q][c]) *
                            fe_v.shape_value_component(i, q, c) * fe_v.JxW(q);
                        }
                    }
    ...
    pcout << "n_laplacian = "
          << Utilities::MPI::sum(n_laplacian, mpi_communicator) << std::endl;

        ...
        case Parameters::AllParameters<dim>::LinearVelocityPotential:
          {
            ...
            velocityPotential::LinearVelocityPotential<dim>
              linear_velocity_potential(triangulation_ptr,
                                        parameters,
                                        mpi_communicator);

            linear_velocity_potential.compute();
            linear_velocity_potential.transfer_solution(fe,
                                                        dof_handler,
                                                        locally_owned_solution);
        ...
        case Parameters::AllParameters<dim>::FullVelocityPotential:
          {
            ...
            velocityPotential::FullVelocityPotential<dim>
              full_velocity_potential(triangulation_ptr,
                                      parameters,
                                      mpi_communicator);

            full_velocity_potential.compute();
            full_velocity_potential.transfer_solution(fe,
                                                      dof_handler,
                                                      locally_owned_solution);
        ...
        case Parameters::AllParameters<dim>::FreeStream:
          {
            ...
            // Compute all three velocity components no matter how may space
            ...
            velocity_infty[1] =
              velocity_in_symm_plan * std::sin(parameters->angle_of_attack);
            velocity_infty[0] =
              velocity_in_symm_plan * std::cos(parameters->angle_of_attack);

            for (unsigned int d = 0; d < dim; ++d)
              {
                free_stream_condition
                  [d + EquationComponents<dim>::first_velocity_component] =
                    velocity_infty[d];
              }
            free_stream_condition[EquationComponents<dim>::density_component] =
              1.0;
            ...
            Functions::ConstantFunction<dim> free_stream_initial_value(
              &free_stream_condition[0], EquationComponents<dim>::n_components);

            VectorTools::interpolate(*mapping_ptr,
                                     dof_handler,
                                     free_stream_initial_value,
                                     locally_owned_solution);
            break;
          }
                            ...
                            if (fe.has_support_on_face(i, face_no) == true)
                              {
                                ...
                                if (component_i != EquationComponents<
                                                     dim>::density_component &&
                                    component_i != EquationComponents<
                                                     dim>::pressure_component)
                                  {
                                    locally_owned_solution[dof_indices[i]] =
                                      0.0;
                                  }
                              }
                          break;
                          ...
                          if (parameters->mms_use_strong_BC)
                            {
                              ...
                              ComponentMask component_mask(
                                EquationComponents<dim>::n_components, true);

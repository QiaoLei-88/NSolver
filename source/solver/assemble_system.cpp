               ...
               ++face_no)
            {
              ...
              for (unsigned int subface_no = 0; subface_no < n_subfaces;
                   ++subface_no)
                {
                  ...
                  // face. There are two cases that we need to distinguish: that
                  // this is a normal face between two cells at the same
                  // refinement level, and that it is a face between two cells
                  // of the different refinement levels.
                  //
                  // In the first case, there is nothing we need to do: we are
                  // using a continuous finite element, and face terms do not
                  // appear in the bilinear form in this case. The second case
                  // usually does not lead to face terms either if we enforce
                  // hanging node constraints strongly (as in all previous
                  // tutorial programs so far whenever we used continuous finite
                  // elements -- this enforcement is done by the
                  // ConstraintMatrix class together with
                  // DoFTools::make_hanging_node_constraints). In the current
                  // program, however, we opt to enforce continuity weakly at
                  // faces between cells of different refinement level, for two
                  // reasons: (i) because we can, and more importantly (ii)
                  // because we would have to thread the automatic
                  // differentiation we use to compute the elements of the
                  // Newton matrix from the residual through the operations of
                  // the ConstraintMatrix class. This would be possible, but is
                  // not trivial, and so we choose this alternative approach.
                  //
                  // What needs to be decided is which side of an interface
                  // between two cells of different refinement level we are
                  // sitting on.
                  //
                  // Let's take the case where the neighbor is more refined
                  // first. We then have to loop over the children of the face
                  // of the current cell and integrate on each of them. We
                  // sprinkle a couple of assertions into the code to ensure
                  // that our reasoning trying to figure out which of the
                  // neighbor's children's faces coincides with a given subface
                  // of the current cell's faces is correct -- a bit of
                  // defensive programming never hurts.
                  //
                  // We then call the function that integrates over faces; since
                  // this is an internal face, the fifth argument is false, and
                  // the sixth one is ignored so we pass an invalid value again:
                  else
                    {
                          ...
                          case -1:
                            {
                              ...
                              face_diameter =
                                neighbor_child->face(neighbor_face_no)
                                  ->diameter();
                              break;
                            }
                          case 0:

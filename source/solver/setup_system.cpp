  ...
  NSolver<dim>::setup_system()
  {
    ...
    if (parameters->output_sparsity_pattern)
      {
        ...
        DoFTools::make_sparsity_pattern(
          ...
          AffineConstraints<typename NSVector::value_type>(),
        ...
        const std::string file_name =
          "sparsity_pattern." +
          Utilities::int_to_string(n_sparsity_pattern_out, 4) + ".origin";
    ...
    if (parameters->max_refine_time > 0.0)
      {
        ...
        //
        // TODO: We need make_flux_sparsity_pattern only on faces with hanging
        // nodes. But currently this function will create cross face connections
        // for all faces.
        //
        // TODO: For Euler equation, we only need to couple DoFs that have
        // support on faces because face flux in weak form is (\phi, F(u) \cdot
        // n), where only values but no gradient of solution are involved.
    ...
    if (parameters->output_sparsity_pattern &&
        parameters->renumber_dofs != Parameters::AllParameters<dim>::None)
      {
        const std::string file_name =
          "sparsity_pattern." +
          Utilities::int_to_string(n_sparsity_pattern_out, 4) + ".renumbered";
        std::ofstream out(file_name.c_str());
        sparsity_pattern.print_gnuplot(out);
      }

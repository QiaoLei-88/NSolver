...
namespace NSFEMSolver
{
  ...
  template <int dim>
  void
  NSolver<dim>::assemble_face_term(
    ...
    const double                                face_diameter)
  {
    ...
    Table<2, Sacado::Fad::DFad<double>> Wminus(
      n_q_points, EquationComponents<dim>::n_components);
    Table<2, double> Wplus_old(n_q_points,
                               EquationComponents<dim>::n_components);
    Table<2, double> Wminus_old(n_q_points,
                                EquationComponents<dim>::n_components);
    ...
    // that would otherwise be tremendously complicated.
    else
      {
        ...
        for (unsigned int q = 0; q < n_q_points; q++)
          {
            // If We call this function with <code>Wminus[q]</code> as last
            // argument, subscripting a 2d table yields a temporary accessor
            // object representing a 1d vector. The problem is that a temporary
            // accessor object can't be bound to a non-const reference argument
            // of a function. So we have to store the non-const accessor object
            // explicitly.
            dealii::internal::TableBaseAccessors::
              Accessor<2, Sacado::Fad::DFad<double>, false, 1>
                Wminus_q = Wminus[q];
            ...
            dealii::internal::TableBaseAccessors::Accessor<2, double, false, 1>
              Wminus_old_q = Wminus_old[q];
            EulerEquations<dim>::compute_Wminus(
              parameters->boundary_conditions[boundary_id].kind,
              fe_v.normal_vector(q),
              Wplus_old[q],
        ...
        for (unsigned int q = 0; q < n_q_points; ++q)
          {
            ...
            std::array<std::array<double, dim>,
                       EquationComponents<dim>::n_components>
              oflux_old;
            EulerEquations<dim>::compute_inviscid_flux(Wminus_old[q],
                                                       oflux_old);
                ...
                for (unsigned int d = 0; d < dim; ++d)
                  {
                    ...
                    normal_fluxes_old[q][c] +=
                      oflux_old[c][d] * normal_vector[d];
                  }
              }
          }
      }
        ...
        for (unsigned int q = 0; q < n_q_points; ++q)
          {
            ...
            EulerEquations<dim>::compute_viscous_flux(
              Wminus[q], grad_W[q], visc_flux, mu, kappa);
            for (unsigned int c = 0; c < EquationComponents<dim>::n_components;
                 ++c)
              {
                for (unsigned int d = 0; d < dim; ++d)
      ...
      if (fe_v.get_fe().has_support_on_face(i, face_no) == true)
        {
          ...
          if (external_face == false)
            {
              system_matrix.add(dof_indices[i],
                                dof_indices_neighbor.size(),
                                &dof_indices_neighbor[0],
                                &(R_i.fastAccessDx(dof_indices.size())));
            }

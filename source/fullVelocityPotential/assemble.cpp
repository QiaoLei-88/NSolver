...
{
  template <int dim>
  void FullVelocityPotential<dim>::assemble_system()
  {
          ...
          for (unsigned int q_point=0; q_point<n_q_points; ++q_point)
            {
              const double density =
                internal::compute_density<dim> (solution_gradients[q_point],gm1,Mach_infty_square);
              const double density_1mc = std::pow (density, 1.0-gm1);
              const double const_b = solution_gradients[q_point] * solution_gradients[q_point] * density_1mc;
                    ...
                    for (unsigned int q_point=0; q_point<n_face_q_points; ++q_point)
                      {
                        ...
                        for (unsigned int i=0; i<dofs_per_cell; ++i)
                          {
                            cell_rhs (i) += face_flux * fe_face_values.shape_value (i,q_point);
                          }
                      }
                    break;
                  }

...
namespace velocityPotential
{
  ...
  template <int dim>
  void
  FullVelocityPotential<dim>::transfer_solution(
    ...
    LA::MPI::Vector &      NS_solution) const
  {
    ...
    Quadrature<dim> quadrature_on_NS_support_points(
      fe_NS.base_element(0).get_unit_support_points());

    FEValues<dim> fe_values_potential(fe_potential,
                                      quadrature_on_NS_support_points,
    ...
    typename DoFHandler<dim>::active_cell_iterator
      ...
      cell_NS = dof_handler_NS.begin_active(), endc_NS = dof_handler_NS.end();
    for (; cell_NS != endc_NS; ++cell_NS, ++cell_potential)
      {
        Assert(
          cell_potential != dof_handler_potential.end(),
          ExcMessage(
            "Reached end of tria for potential equation before NS equation!"));
        if (cell_NS->is_locally_owned())
          {
            fe_values_potential.reinit(cell_potential);

            fe_values_potential.get_function_gradients(potential_solution,
                                                       potential_grad_on_cell);

            cell_NS->get_dof_indices(global_indices_of_local_dofs);
            // Since the quadrature is extracted according to the target FE
            // supporting points, we just have to loop through all the
            // quadrature points. We assume that the quadrature points and
            // supporting points are arranged in the same order.
            for (unsigned int q_point = 0; q_point < n_q_points; ++q_point)
              {
                for (unsigned d  = 0,
                              ic = NSFEMSolver::EquationComponents<
                                dim>::first_velocity_component;
                     d < dim;
                     ++d, ++ic)
                  {
                    const unsigned int system_index =
                      fe_NS.component_to_system_index(ic, q_point);
                    NS_solution[global_indices_of_local_dofs[system_index]] =
                      potential_grad_on_cell[q_point][d];
                    ;
                  }
                const double density = internal::compute_density<dim>(
                  potential_grad_on_cell[q_point], gm1, Mach_infty_square);
                {
                  // Density component
                  const unsigned int ic =
                    NSFEMSolver::EquationComponents<dim>::density_component;
                  const unsigned int system_index =
                    fe_NS.component_to_system_index(ic, q_point);
                  NS_solution[global_indices_of_local_dofs[system_index]] =
                    density;
                }
                {
                  // Pressure component
                  const unsigned int ic =
                    NSFEMSolver::EquationComponents<dim>::pressure_component;
                  const unsigned int system_index =
                    fe_NS.component_to_system_index(ic, q_point);
                  NS_solution[global_indices_of_local_dofs[system_index]] =
                    std::pow(density, gas_gamma) / gas_gamma;
                }
              }
          }
      }

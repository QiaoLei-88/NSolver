...
namespace velocityPotential
{
  ...
  template <int dim>
  void FullVelocityPotential<dim>::transfer_solution (
        ...
        if (cell_NS->is_locally_owned())
          {
            ...
            for (unsigned int q_point=0; q_point<n_q_points; ++q_point)
              {
                for (unsigned d=0, ic = NSFEMSolver::EquationComponents<dim>::first_velocity_component;
                     d<dim;
                     ++d, ++ic)
                  {
                    const unsigned int system_index = fe_NS.component_to_system_index (ic, q_point);
                    NS_solution[global_indices_of_local_dofs[system_index]]
                      = potential_grad_on_cell[q_point][d];;
                  }
                const double density =
                  internal::compute_density<dim> (potential_grad_on_cell[q_point],gm1,Mach_infty_square);
                {
                  // Density component
                  const unsigned int ic = NSFEMSolver::EquationComponents<dim>::density_component;
                  const unsigned int system_index = fe_NS.component_to_system_index (ic, q_point);
                  NS_solution[global_indices_of_local_dofs[system_index]] = density;
                }
                {
                  //Pressure component
                  const unsigned int ic = NSFEMSolver::EquationComponents<dim>::pressure_component;
                  const unsigned int system_index = fe_NS.component_to_system_index (ic, q_point);
                  NS_solution[global_indices_of_local_dofs[system_index]] =

...
namespace NSFEMSolver
{
  ...
  void
  CellDataTransfer<dim, InternalValueType>::get_transfered_data(
    ...
    UserValueType *    data_dest) const
  {
    ...
    Assert(!(active_data_size > tria.n_active_cells()),
           ExcMessage(
             "Tracked active cell number is greater than actual value"));
  ...
  template void
  CellDataTransfer<2, double>::get_transfered_data<double>(
    const unsigned int index,
    double *           data_dest) const;
  template void
  CellDataTransfer<2, double>::get_transfered_data<float>(
    const unsigned int index,
    float *            data_dest) const;
  template void
  CellDataTransfer<3, double>::get_transfered_data<double>(
    const unsigned int index,
    double *           data_dest) const;
  template void
  CellDataTransfer<3, double>::get_transfered_data<float>(
    const unsigned int index,
    float *            data_dest) const;

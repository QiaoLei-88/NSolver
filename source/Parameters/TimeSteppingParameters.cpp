...
namespace NSFEMSolver
{
  namespace Parameters
  {
    ...
    void
    TimeStepping::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("time stepping");
      {
        ...
        prm.declare_entry("rigid reference time step",
                          "true",
                          Patterns::Bool(),
                          "whether use specified reference time step or"
                          "calculate according to CFL condition.");
        prm.declare_entry("reference time step",
                          "0.1",
                          Patterns::Double(),
        ...
        prm.declare_entry("CFL number",
                          ...
                          Patterns::Double(0),
                          "CFL number");
        prm.declare_entry("CFL number max",
                          "-1.0",
                          Patterns::Double(),
                          "upper limit of CFL number");
        prm.declare_entry("CFL number min",
                          "-1.0",
                          Patterns::Double(),
                          "minimum of CFL number");
        prm.declare_entry("allow increase CFL",
                          "true",
                          Patterns::Bool(),
                          "allow increase CFL number for unsteady simulation");
        prm.declare_entry("allow decrease CFL",
                          "true",
                          Patterns::Bool(),
    ...
    void
    TimeStepping::parse_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("time stepping");
      {
        final_time      = prm.get_double("final time");
        max_Newton_iter = prm.get_integer("max Newton iter per time step");
        solution_update_l2_tolerance =
          prm.get_double("solution update l2 tolerance");
        physical_residual_l2_tolerance =
          prm.get_double("physical residual l2 tolerance");
        ...
        AssertThrow(reference_time_step != 0.0,
                    ExcMessage(" Time step size can't be 0."));
        is_steady = false;
        ...
        step_increasing_ratio_stage1 =
          prm.get_double("step increasing ratio stage1");
        minimum_step_increasing_ratio_stage2 =
          prm.get_double("minimum step increasing ratio stage2");
        step_increasing_power_stage2 =
          prm.get_double("step increasing power stage2");


        solution_extrapolation_length =
          prm.get_double("solution extrapolation length");

        newton_linear_search_length_try_limit =
          prm.get_integer("newton linear search length try limit");
        step_with_physical_residual =
          prm.get_bool("step with physical residual");
      }
      prm.leave_subsection();
    }

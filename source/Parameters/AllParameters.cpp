  ...
  namespace Parameters
  {
    ...
    Solver::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("linear solver");
      {
        ...
        prm.declare_entry("RCM reorder",
                          "false",
                          Patterns::Bool(),
                          "Do Reverse Cuthill–McKee reordering.");
        prm.declare_entry("AZ_kspace",
                          "150",
                          Patterns::Integer(0),
    ...
    template <int dim>
    void
    Refinement<dim>::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("refinement");
      {
        prm.declare_entry("refinement indicator",
                          "Gradient",
                          Patterns::Selection("Gradient|Kelly|ErrorAndJump"),
                          "type of refinement indicator");
        prm.declare_entry("do initial refine",
                          "true",
                          Patterns::Bool(),
                          "Do refine on initial field or not");
        prm.declare_entry("refine fraction",
                          "0.1",
                          Patterns::Double(),
                          "Fraction of gird refinement");
        prm.declare_entry("coarsen fraction",
                          "0.1",
                          Patterns::Double(),
        ...
        prm.declare_entry("min cell size",
                          "-1.0",
                          Patterns::Double(),
    ...
    template <int dim>
    void
    Refinement<dim>::parse_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("refinement");
      {
        {
    ...
    Flux::declare_parameters(ParameterHandler &prm)
    {
      ...
      {
        prm.declare_entry("flux type",
                          "LaxFriedrichs",
                          Patterns::Selection("LaxFriedrichs|Roe"),
                          "Numerical flux type");
        prm.declare_entry("flux type switch to",
                          "Roe",
                          Patterns::Selection("LaxFriedrichs|Roe"),
                          "Numerical flux type");
    ...
    AllParameters<dim>::declare_parameters(ParameterHandler &prm)
    {
      ...
      prm.declare_entry("mesh format",
                        "gmsh",
                        Patterns::Anything(),
                        "input mesh file format");

      prm.declare_entry("scale mesh",
                        "1.0",
                        Patterns::Double(),
      ...
      prm.declare_entry("time history",
                        "time_history.out",
                        Patterns::Anything(),
                        "output file for time history.");

      prm.declare_entry("iteration history",
                        "iter_history.out",
                        Patterns::Anything(),
                        "output file for all iteration history.");

      prm.declare_entry("renumber dofs",
                        "None",
                        Patterns::Selection("None|RCM|RCM_WithStartPoint"),
                        "How to renumber dofs");

      prm.declare_entry("renumber start point x",
                        "0",
                        Patterns::Double(),
                        "x coordinate of renumber start point");
      prm.declare_entry("renumber start point y",
                        "0",
                        Patterns::Double(),
                        "y coordinate of renumber start point");
      prm.declare_entry("renumber start point z",
                        "0",
                        Patterns::Double(),
                        "z coordinate of renumber start point");
      prm.declare_entry("output sparsity pattern",
                        "false",
                        Patterns::Bool(),
                        "output sparsity pattern or not");
      prm.declare_entry("output system matrix",
                        "false",
                        Patterns::Bool(),
      ...
      for (unsigned int b = 0; b < max_n_boundaries; ++b)
        {
          prm.enter_subsection("boundary_" + Utilities::int_to_string(b));
          {
            ...
            for (unsigned int di = 0;
                 di < EquationComponents<dim>::n_components;
                 ++di)
              {
            ...
            prm.declare_entry("integrate force",
                              "false",
                              Patterns::Bool(),
    ...
    AllParameters<dim>::parse_parameters(ParameterHandler &prm)
    {
           ...
           ++boundary_id)
        {
                               ...
                               Utilities::int_to_string(boundary_id));
          {
            std::vector<std::string> expressions(
              EquationComponents<dim>::n_components, "0.0");
      ...
      prm.enter_subsection("initial condition");
      {
        ...
        AssertThrow(
          init_fe_degree < 9,
          ExcMessage(
            "Do not use element order higher than 8 for potential equation."));
        if (init_fe_degree < 1)
          {
            init_fe_degree = 1;
          }

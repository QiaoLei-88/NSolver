...
namespace NSFEMSolver
{
  namespace Parameters
  {
    ...
    template <int dim>
    void
    StabilizationParameters<dim>::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("stabilization parameters");
      {
        ...
        prm.declare_entry("diffusion coefficient",
                          "1.0",
                          Patterns::Double(),
        ...
        prm.declare_entry("Laplacian continuation",
                          "-1.0",
                          Patterns::Double(),
                          "Coefficient for Laplacian continuation");
        prm.declare_entry(
          "compute laplacian coeff from Mach max",
          "false",
          Patterns::Bool(),
          "Compute laplacian coefficient from maximum Mach number among entire flow field");
        prm.declare_entry("enable partial laplacian",
                          "false",
                          Patterns::Bool(),
                          "Enable partial laplacian");
        prm.declare_entry(
          "dodge mesh adaptation",
          "false",
          Patterns::Bool(),
        ...
        prm.declare_entry(
          "Laplacian Newton tolerance",
          "0.1",
          Patterns::Double(0.0),
          "Threshold of Laplacian Coefficient to be force to zero");

        prm.declare_entry("SUPG factor",
                          "-1.0",
                          Patterns::Double(),
                          "SUPG stabilization factor.");

        for (unsigned int di = 0; di < EquationComponents<dim>::n_components;
             ++di)
          {
            prm.declare_entry("diffusion factor for w_" +
                                Utilities::int_to_string(di),
                              "1.0",
                              Patterns::Double(0),
                              "diffusion factor for component w_" +
                                Utilities::int_to_string(di));
          }
      }
    ...
    template <int dim>
    void
    StabilizationParameters<dim>::parse_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("stabilization parameters");
      {
        {
        ...
        oscillation_visc_background =
          prm.get_double("oscillation visc background");
        oscillation_visc_coefficient =
          prm.get_double("oscillation visc coefficient");
        oscillation_with_characteristic_speed =
          prm.get_bool("oscillation with characteristic speed");

        smooth_artificial_viscosity =
          prm.get_bool("smooth artificial viscosity");
        ...
        continuation_min_decrease_rate =
          prm.get_double("continuation min decrease rate");
        continuation_decrease_residual_power =
          prm.get_double("continuation decrease residual power");
        continuation_blend_starting_ratio =
          prm.get_double("continuation blend starting ratio");
        ...
        count_solution_diff_in_residual = prm.get_bool("CountSolDiffInRes");
        ...
        compute_laplacian_coeff_from_Mach_max =
          prm.get_bool("compute laplacian coeff from Mach max");
        ...
        laplacian_newton_tolerance =
          prm.get_double("Laplacian Newton tolerance");

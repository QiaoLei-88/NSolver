...
namespace NSFEMSolver
{
  namespace Parameters
  {
    ...
    void
    FEParameters::declare_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("FE Parameters");
      {
        prm.declare_entry("mapping type",
                          "MappingQ",
        ...
        prm.declare_entry("mapping degree",
                          "-1",
                          Patterns::Integer(),
                          "Mapping degree");

        prm.declare_entry("quadrature degree",
                          "-1",
                          Patterns::Integer(),
                          "Cell quadrature degree");

        prm.declare_entry("face quadrature degree",
                          "-1",
                          Patterns::Integer(),
                          "Face quadrature degree");

        prm.declare_entry("error quadrature degree",
                          "-1",
                          Patterns::Integer(),
    ...
    void
    FEParameters::parse_parameters(ParameterHandler &prm)
    {
      prm.enter_subsection("FE Parameters");
      {
        {

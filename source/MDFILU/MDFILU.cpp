...
void
MDFILU::reinit(const MDFILU::global_index_type estimated_row_length_in,
...
bool
MDFILU::Indicator::operator<(const Indicator &op) const
{
...
MDFILU::global_index_type
MDFILU::get_info_of_non_zeros(const global_index_type row_to_factor,
                              std::vector<EntryInfo> &incides_need_update,
                              const bool              except_pivot) const
{
  global_index_type n_non_zero = 0;

  typename DynamicMatrix::const_iterator iter_fill =
    fill_in_level.begin(row_to_factor);
...
void
MDFILU::compute_discarded_value(const unsigned int row_to_factor,
                                const bool         update)
{
...
#  endif
  ...
  if (pivot == 0.0)
  ...
  else
    {
      ...
      // During this procedure, we need to go through all un-factored entries
      // that connected with this row, i.e., for all k that a(i_row, k) \ne 0
      // and a(k, i_row) \ne 0. That's why we need the flag array row_factored.
              ...
              if (new_fill_in_level == 0 /* fill in level for new entry*/)
                {
                  ...
                  new_fill_in_level = (incides_need_update[j].fill_level -
                                       fill_in_level_for_original_entry) +
                                      (fill_level_of_row_pivot -
                                       fill_in_level_for_original_entry) +
                                      1 + fill_in_level_for_original_entry;
                }
              if (new_fill_in_level > fill_in_threshold &&
                  i_row != j_col) // Never drop diagonal element
                {
                  // Element will be discarded
                  const data_type update =
                    incides_need_update[j].value * value_of_row_pivot;
...
void
MDFILU::MDF_reordering_and_ILU_factoring()
{
#  ifdef VERBOSE_OUTPUT
...
#  endif
  ...
  for (global_index_type n_row_factored = 0; n_row_factored < degree;
       ++n_row_factored)
    {
      // Find the row with minimal discarded value
      ILU_timer.enter_subsection("Find min discarded value");
      const global_index_type row_to_factor = sorted_indicators.begin()->index;
          ...
          for (global_index_type j = 0; j < n_row_need_update; ++j)
            {
              ...
              // Make sure that the provided fill_in_threshold consists with
              // the internal definition, i.e., has a offset one. See
              ...
              if (new_fill_in_level <= fill_in_threshold ||
                  i_row == j_col) // Always keep diagonal element)
                {
                  ...
                  const data_type update =
                    value - incides_need_update[j].value * value_of_row_pivot;
...
int
MDFILU::apply(const data_type *const in, data_type *const out) const
{
...
int
MDFILU::apply_transpose(const data_type *const in, data_type *const out) const
{
  Assert(metrix_factored, ExcMessage("Call factor_matrix() before using it!"));
      ...
      for (; p_data < p_end; ++p_data, ++p_pattern)
        {
          ...
          if (j < i)
            {
              // Lower triangle only
              // Because the i-loop goes up from i=0, all j<i must have
              // experienced j==i in previous i-loop, with the assumption that
              // diagonal entry exists in all rows. So it is save to use +=
              // here.
      ...
      for (; p_data < p_end; ++p_data, ++p_pattern)
        {
          ...
          if (j > i)
            {
              // Upper triangle only
              // Because the i-loop goes down from top, all j>i must have
              // experienced j==i in previous i-loop, with the assumption that
              // diagonal entry exists in all rows. So it is save to use +=
              // here.
            ...
            {
              // We cannot overwrite out[i_row] immediately because it is used
              // above and our matrix is permuted. We don't know which if branch
              // comes first.
              pivot = (*p_data);
            }
        }
...
int
MDFILU::apply_inverse(const data_type *const in, data_type *const out) const
{
  Assert(metrix_factored, ExcMessage("Call factor_matrix() before using it!"));
  ...
  for (global_index_type i = 0; i < degree; ++i)
    {
      ...
      // Diagonal value of L is alway 1, so we can just accumulate on
      // out[i_row].
      out[i_row] = in[i_row];
...
int
MDFILU::apply_inverse_transpose(const data_type *const in,
                                data_type *const       out) const
{
  Assert(metrix_factored, ExcMessage("Call factor_matrix() before using it!"));
      ...
      for (; p_data < p_end; ++p_data, ++p_pattern)
        {
          ...
          if (j == i)
            {
              Assert((*p_data) != 0.0, ExcMessage("Zero pivot encountered!"));
...
int
MDFILU::apply(const Vector<data_type> &in, Vector<data_type> &out) const
{
  Assert(in.size() == out.size(), ExcDimensionMismatch(in.size(), out.size()));
  Assert(in.size() == degree, ExcDimensionMismatch(in.size(), degree));
...
int
MDFILU::apply_inverse(const Vector<data_type> &in, Vector<data_type> &out) const
{
  Assert(in.size() == out.size(), ExcDimensionMismatch(in.size(), out.size()));
  Assert(in.size() == degree, ExcDimensionMismatch(in.size(), degree));
...
int
MDFILU::apply(const NSVector &in, NSVector &out) const
{
  Assert(in.size() == out.size(), ExcDimensionMismatch(in.size(), out.size()));
  Assert(in.size() == degree, ExcDimensionMismatch(in.size(), degree));
...
int
MDFILU::apply_inverse(const NSVector &in, NSVector &out) const
{
  Assert(in.size() == out.size(), ExcDimensionMismatch(in.size(), out.size()));
  Assert(in.size() == degree, ExcDimensionMismatch(in.size(), degree));
...
int
MDFILU::Apply(const Epetra_MultiVector &in, Epetra_MultiVector &out) const
{
...
int
MDFILU::ApplyInverse(const Epetra_MultiVector &in,
                     Epetra_MultiVector &      out) const
{
  Assert(in.NumVectors() == out.NumVectors(),
         ExcDimensionMismatch(in.NumVectors(), out.NumVectors()));

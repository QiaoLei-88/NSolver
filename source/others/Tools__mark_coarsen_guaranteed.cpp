...
namespace NSFEMSolver
{
  ...
  namespace Tools
  {
                            ...
                            const std::vector<short int> &  coarsen_mask)
    {
      ...
      // Coarsen all short cut is removed because the original code is
      // erroneous. If we define maximum coarsening as removing all fine all
      // refine flags and setting all coarsen flags. The old code used maximum
      // coarsening
      ...
      // case since we have to count on the contribution of canceling
      // refinements, and only part of refinements are needed to be canceled in
      // most cases.
    ...
    // Instantiation
    template void mark_coarsen_guaranteed<2, float>(
      parallel::distributed::Triangulation<2> &,
      const Vector<float> &,
      const std::pair<double, double>,
      const unsigned int,
      const std::vector<short int> &);
    template void mark_coarsen_guaranteed<2, double>(
      parallel::distributed::Triangulation<2> &,
      const Vector<double> &,
      const std::pair<double, double>,
      const unsigned int,
      const std::vector<short int> &);
    template void mark_coarsen_guaranteed<3, float>(
      parallel::distributed::Triangulation<3> &,
      const Vector<float> &,
      const std::pair<double, double>,
      const unsigned int,
      const std::vector<short int> &);
    template void mark_coarsen_guaranteed<3, double>(
      parallel::distributed::Triangulation<3> &,
      const Vector<double> &,
      const std::pair<double, double>,
      const unsigned int,

...
namespace NSFEMSolver
{
  ...
  namespace Tools
  {
      ...
      if (n_cell_after_refine > parameters->max_cells)
        {
          ...
          std::pair<double, double> global_min_and_max;
          {
            ...
            global_min_and_max.first = static_cast<double>(
              Utilities::MPI::min(local_min, mpi_communicator));
            global_min_and_max.second = static_cast<double>(
    ...
    // Instantiation
    template void refine_on_threshold<2, Vector<float>>(
      parallel::distributed::Triangulation<2> &,
      const Vector<float> &,
      const SmartPointer<Parameters::AllParameters<2> const> &,
      const float);

    template void refine_on_threshold<2, Vector<double>>(
      parallel::distributed::Triangulation<2> &,
      const Vector<double> &,
      const SmartPointer<Parameters::AllParameters<2> const> &,
      const double);

    template void refine_on_threshold<3, Vector<float>>(
      parallel::distributed::Triangulation<3> &,
      const Vector<float> &,
      const SmartPointer<Parameters::AllParameters<3> const> &,
      const float);

    template void refine_on_threshold<3, Vector<double>>(
      parallel::distributed::Triangulation<3> &,
      const Vector<double> &,
      const SmartPointer<Parameters::AllParameters<3> const> &,
      const double);

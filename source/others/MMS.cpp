...
namespace NSFEMSolver
{
  ...
  template <int dim>
  double
  MMS<dim>::value(const Point<dim> &p, const unsigned int component) const
  {
    double                  rv(0.0);
  ...
  template <int dim>
  void
  MMS<dim>::vector_value(const Point<dim> &p, Vector<double> &value) const
  {
  ...
  template <int dim>
  void
  MMS<dim>::value_list(const std::vector<Point<dim>> &point_list,
  ...
  template <int dim>
  void
  MMS<dim>::vector_value_list(const std::vector<Point<dim>> &point_list,
  ...
  template <int dim>
  void
  MMS<dim>::evaluate(const Point<dim> &p,
                     F_V &             value,
                     F_T &             grad,
                     F_V &             source,
  ...
  {
    Assert(initialized,
           ExcMessage("run MMS::reinit(...) before MMS::evaluation(...)."));
        ...
        if (is_NS)
          {
            ...
            EulerEquations<dim>::compute_viscous_flux(
              value_ad, grad_value, visc_flux, 0, 0);
  ...
  void
  MMS<2>::value_at_point(const std::array<Number, 2> &p,
  ...
  template <typename Number>
  void
  MMS<3>::value_at_point(const std::array<Number, 3> &p,
                         const unsigned int           component,
                         Number &                     result) const
  {

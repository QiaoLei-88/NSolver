...
namespace NSFEMSolver
{
  ...
  namespace Tools
  {
      ...
      const SmartPointer<Parameters::AllParameters<dim> const> &parameters)
    {
      ...
      std::pair<double, double> global_min_and_max;
      {
        ...
        for (; cell != endc; ++cell)
          if (cell->is_locally_owned())
            {
              local_min =
                std::min(local_min, criteria[cell->active_cell_index()]);
              local_max =
                std::max(local_max, criteria[cell->active_cell_index()]);
            }
      ...
      const unsigned int n_refined = mark_refine_guaranteed<dim, VType>(
        tria, criteria, global_min_and_max, top_fraction_of_cells, refine_mask);
      ...
      if (n_cell_after_refine > parameters->max_cells)
        {
          ...
          mark_coarsen_guaranteed<dim, VType>(
            tria, criteria, global_min_and_max, n_cell_exceeded, coarsen_mask);
        }
      else
        {
          // Otherwise, if the planed refinement is not going to break the max
          // cell number limit, we just need to mark cell to coarsen blindly, no
          // matter the cell will really coarsen or not.
    ...
    // Instantiation
    template void refine_and_coarsen_fixed_number<2, Vector<float>>(
      parallel::distributed::Triangulation<2> &,
      const Vector<float> &,
      const SmartPointer<Parameters::AllParameters<2> const> &);

    template void refine_and_coarsen_fixed_number<2, Vector<double>>(
      parallel::distributed::Triangulation<2> &,
      const Vector<double> &,
      const SmartPointer<Parameters::AllParameters<2> const> &);

    template void refine_and_coarsen_fixed_number<3, Vector<float>>(
      parallel::distributed::Triangulation<3> &,
      const Vector<float> &,
      const SmartPointer<Parameters::AllParameters<3> const> &);

    template void refine_and_coarsen_fixed_number<3, Vector<double>>(
      parallel::distributed::Triangulation<3> &,
      const Vector<double> &,
      const SmartPointer<Parameters::AllParameters<3> const> &);

  ...
  namespace Tools
  {
                           ...
                           const std::vector<short int> &  refine_mask)
    {
      ...
      const unsigned int n_target_cells = static_cast<unsigned int>(
        target_fraction * tria.n_global_active_cells() + 0.5);
      ...
      MPI_Bcast(
        &total_count, 1, MPI_UNSIGNED, master_mpi_rank, mpi_communicator);
      return (total_count);
    }

    // Instantiation
    template unsigned int mark_refine_guaranteed<2, float>(
      parallel::distributed::Triangulation<2> &,
      const Vector<float> &,
      const std::pair<double, double>,
      const double,
      const std::vector<short int> &);
    template unsigned int mark_refine_guaranteed<2, double>(
      parallel::distributed::Triangulation<2> &,
      const Vector<double> &,
      const std::pair<double, double>,
      const double,
      const std::vector<short int> &);
    template unsigned int mark_refine_guaranteed<3, float>(
      parallel::distributed::Triangulation<3> &,
      const Vector<float> &,
      const std::pair<double, double>,
      const double,
      const std::vector<short int> &);
    template unsigned int mark_refine_guaranteed<3, double>(
      parallel::distributed::Triangulation<3> &,
      const Vector<double> &,
      const std::pair<double, double>,
      const double,

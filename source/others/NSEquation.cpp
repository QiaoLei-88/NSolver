...
namespace NSFEMSolver
{
  ...
  template <int dim>
  void
  EulerEquations<dim>::set_parameter(
    const Parameters::AllParameters<dim> *const para_ptr_in)
  {
  ...
  {
    Assert(component_mask.size() >= n_components,
           ExcMessage("component_mask doesn't have enough elements"));
          ...
          for (unsigned int ic = 0; ic < n_components; ++ic)
            {
              if (component_mask[ic])
                {
                  refinement_indicators[cell_no] += static_cast<float>(
                    std::log(1.0 + std::sqrt(dU[0][ic] * dU[0][ic])));
                }
            }
        }
  }
  ...
  template <int dim>
  Parameters::AllParameters<dim> const *EulerEquations<dim>::parameters = 0;

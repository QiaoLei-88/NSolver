...
namespace velocityPotential
{
  template <int dim>
  void
  LinearVelocityPotential<dim>::Postprocessor::evaluate_scalar_field(
    const DataPostprocessorInputs::Scalar<dim> &inputs,
    std::vector<Vector<double>> &               computed_quantities) const
  {
    const unsigned int n_quadrature_points =
      static_cast<const unsigned int>(inputs.solution_values.size());
    ...
    for (unsigned int q = 0; q < n_quadrature_points; ++q)
      {
        ...
        for (unsigned int d = 0; d < dim; ++d)
          {
            computed_quantities[q][d] =
              velocity_infty[d] + inputs.solution_gradients[q][d];
        ...
        const double Mach_ratio = (1.0 + 0.5 * (gas_gamma - 1.0) * Mach_infty) /
  ...
  LinearVelocityPotential<dim>::Postprocessor::get_names() const
  {
  ...
  LinearVelocityPotential<
    dim>::Postprocessor::get_data_component_interpretation() const
  {
    std::vector<DataComponentInterpretation::DataComponentInterpretation>
      interpretation(dim,
  ...
  template <int dim>
  UpdateFlags
  LinearVelocityPotential<dim>::Postprocessor::get_needed_update_flags() const
  {

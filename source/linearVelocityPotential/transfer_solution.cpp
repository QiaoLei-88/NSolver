...
namespace velocityPotential
{
  ...
  template <int dim>
  void
  LinearVelocityPotential<dim>::transfer_solution(
            ...
            for (unsigned int q_point = 0; q_point < n_q_points; ++q_point)
              {
                ...
                for (unsigned d = 0; d < dim; ++d)
                  {
                    ...
                    const unsigned int system_index =
                      fe_NS.component_to_system_index(d, q_point);
                  ...
                  (1.0 + 0.5 * (gas_gamma - 1.0) * Mach_local);
                {
                  // Density component
                  const unsigned int system_index =
                    fe_NS.component_to_system_index(dim, q_point);
                  NS_solution[global_indices_of_local_dofs[system_index]] =
                ...
                }
                {
                  // Pressure component
                  const unsigned int system_index =
                    fe_NS.component_to_system_index(dim + 1, q_point);
                  NS_solution[global_indices_of_local_dofs[system_index]] =
                    std::pow(Mach_ratio, gas_gamma / (gas_gamma - 1.0)) /
                    gas_gamma;
                }
              }
          }

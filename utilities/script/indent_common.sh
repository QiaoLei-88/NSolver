...
checks() {
  if test ! -d source -o ! -d header -o ! -d tests ; then
...
process_changed()
{
  if [ "x$INDENT_ANCESTOR" == "x" ]; then
    LAST_MERGE_COMMIT="$(git log --format="%H" --merges --max-count=1 master)"
    INDENT_ANCESTOR="$(git merge-base "${LAST_MERGE_COMMIT}" HEAD)"
  fi
  ...
  ( git ls-files --others --exclude-standard -- ${1};
    git diff --name-only $INDENT_ANCESTOR -- ${1} )|
      sort -u |

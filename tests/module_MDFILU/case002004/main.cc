...
// #endif
  ...
  for (unsigned level = 0; level < 3; ++level)
    {
      mdfilu.reinit (estimated_row_length, level);
      // Out put LU
      {
        std::ofstream fout ("LU.out", (level==0 ? std::fstream::trunc : std::fstream::app));
        fout << "\nfill_in_level = " << level << std::endl << std::endl;
      ...
      // For now the sparsity pattern is ignored.
      {
        ...
        std::ofstream fout ("A.out", (level==0 ? std::fstream::trunc : std::fstream::app));
        fout << "\nfill_in_level = " << level << std::endl << std::endl;
        A.print (fout);
        fout.close();
      }
      ...
      {
        std::ofstream fout ("apply.out", (level==0 ? std::fstream::trunc : std::fstream::app));
        fout << "\nfill_in_level = " << level << std::endl << std::endl;

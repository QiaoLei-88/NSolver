...
namespace NSFEMSolver
{
  ...
  public:
     ...
     * so that its contents could be transfered to the new mesh during mesh
     * adaptation. The vector handed in will be assigned an index automatically
     * which is the return value of this function. The index starts from 0 and
     * increases 1 after another so assign multiple vectors is allowed and value
     * types of the vectors don't have to be the same. You need to provide
     * correct index to get_transfered_data() to retrieve the transfered data.
     *
     ...
     * All its data in range [0, tria.n_active_cell()) will be cached
     * internally.
     *
    ...
    unsigned int
    push_back(UserValueType *const data_src, const size_type size);
     ...
     * the type of InternalValueType or corresponding provided vector. Implicit
     * type conversion applies.
     */
    template <typename UserValueType>
    void
    get_transfered_data(const unsigned int index,
  ...
  private:
     ...
     * Triangulation::signals.pre_coarsening_on_cell.
     */
    virtual void
     ...
     * This function is connected to
     * Triangulation::signals.post_refinement_on_cell.
     */
    virtual void
    children_to_parent(const typename Triangulation<dim>::cell_iterator &cell);

    /**
     * Cell could be created, deleted, activated or deactivated during
     * adaptation. New memory will be allocated for data corresponds to newly
     * created or activated cells, but data corresponds to deleted or
     * deactivated cells will not be destroyed immediately. This counter is used
     * to keep track on the total number of active data. This is not necessary
     * for data transferring, only used for safety checks.
     */
    ...
    /**
     * A constant reference to the related triangulation that used to loop
     * through all cells.
     */
    const Triangulation<dim> &tria;

    /**
     * Signal connections between the related triangulation. We need to store
     * them in order to disconnect before destruction.
     */

...
namespace NSFEMSolver
{
  ...
  namespace Tools
  {
     ...
     * p4est has known issue about extra cell refinements, the actual cell refine
     * number may be more than the return value.
     *
     ...
     * in several cases cell coarsening will be canceled by deal.II mesh smoothing,
     * the cells marked by this function are not guaranteed to coarsen.
     ...
     * its value in @p criteria.
     *
     * Because deal.II mesh smoothing is not considered, this function is a little
     * bit faster than mark_coarsen_guaranteed().
     */
    template <int dim, typename Number>
    void
     ...
     *
     * First, the requested refine fraction is marked with function mark_refine_guaranteed.
     * After this, if cell number is going to exceed the max cell number limit,
     * then proper fraction of cell will be marked to coarsen by function
     * mark_coarsen_guaranteed to keep total cell number under the limit.
     * Otherwise, coarsening marking will be done with function mark_coarsen_blindly.
     *
     * @note max cell number limit is not guaranteed. Although deal.II mesh smoothing
     * is taken in to consider, however the p4est mesh smoothing is not under control.
     * Beside this, usually it is impossible to find refine and coarsen thresholds
     * that result a cell number exactly equal to the desired value.
     */

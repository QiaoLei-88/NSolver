...
private:
  ...
  public:
    ...
    init();
    bool
    operator<(const Indicator &op) const;
    bool
    operator==(const Indicator &op) const;
  };
...
public:
  ...
  void
  reinit(const global_index_type estimated_row_length_in,
         const global_index_type fill_in_threshold_in);

  global_index_type
  number_of_new_fill_ins() const;

  int
  apply(const data_type *const in, data_type *const out) const;
  int
  apply_transpose(const data_type *const in, data_type *const out) const;

  int
  apply_inverse(const data_type *const in, data_type *const out) const;
  int
  apply_inverse_transpose(const data_type *const in,
                          data_type *const       out) const;

  // Dedicated interface to dealii Vector<double>
  int
  apply(const Vector<data_type> &in, Vector<data_type> &out) const;
  int
  apply_inverse(const Vector<data_type> &in, Vector<data_type> &out) const;

  int
  apply(const NSVector &in, NSVector &out) const;
  int
  apply_inverse(const NSVector &in, NSVector &out) const;

  // Interface for debug code
  const std::vector<global_index_type> &
  ...
  virtual int
  Apply(const Epetra_MultiVector &, Epetra_MultiVector &) const;
  virtual int
...
private:
  ...
  // During factoring procedure, we need to go through all un-factored entries
  ...
  global_index_type
  get_info_of_non_zeros(const global_index_type row_to_factor,
                        std::vector<EntryInfo> &incides_need_update,
                        const bool              except_pivot) const;

  void
  compute_discarded_value(const unsigned int row_to_factor, const bool update);
...
inline const std::vector<MDFILU::global_index_type> &
MDFILU::get_permutation() const
{
  return (permute_logical_to_storage);
}

inline const MDFILU::DynamicMatrix &
MDFILU::get_LU() const
{
  return (LU);
}

inline MDFILU::global_index_type
MDFILU::number_of_new_fill_ins() const
{
...
inline double
MDFILU::NormInf() const
{
  return (MDFILU::very_large_number);
}

inline bool
MDFILU::HasNormInf() const
{
  return (has_norm_infty);
}

inline bool
MDFILU::UseTranspose() const
{
...
inline int
MDFILU::SetUseTranspose(const bool in)
{
  use_transpose = in;
  return (0);
}
...
MDFILU::OperatorDomainMap() const
{

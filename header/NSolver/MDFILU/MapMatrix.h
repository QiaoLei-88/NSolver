...
public:
  ...
  public:
    ...
    operator++();
    bool
    operator==(const Accessor &op) const;
    bool
    operator!=(const Accessor &op) const;
  ...
  void
  copy_from(const SourceMatrix &matrix, bool);
  void
  clear();
  void
  reinit(const size_type row_szie,
         const size_type column_size,
         const size_type fake);
  Number
   ...
   * Use this function when you know the row is not compressed to avoid if
   * evaluation.
   */
  Number
  map_el(const size_type i, const size_type j) const;
  /*
   ...
   */
  void
  map_set(const size_type i, const size_type j, const Number value);

  const Iterator
  begin(const size_type row) const;
  const Iterator
  end(const size_type row) const;
  const Iterator
  diagonal(const size_type row) const;

  void
  compress(const size_type row);
  const size_type *
  begin_compressed_pattern(const size_type row) const;
  const Number *
  begin_compressed_data(const size_type row) const;
  const size_type *
  end_compressed_pattern(const size_type row) const;
  const Number *
  end_compressed_data(const size_type row) const;
  ...
  if (compressed_data[i].size() > 0)
    {
      ...
      std::vector<size_type>::const_iterator it =
        Utilities::lower_bound(compressed_pattern[i].begin(),
      ...
      if (it != compressed_pattern[i].end() && *it == j)
        {
          return (
            compressed_data[i]
                           [std::distance(compressed_pattern[i].begin(), it)]);
        }
      else
        {
          return (0.0);
...
template <typename Number>
inline void
MapMatrix<Number>::set(const size_type i,
                       const size_type j,
                       const Number    value,
                       bool)
{
  if (compressed_data[i].size() > 0)
    {
      ...
      compressed_data[i][std::distance(compressed_pattern[i].begin(), it)] =
        value;
    }
  else
    {
...
template <typename Number>
inline void
MapMatrix<Number>::map_set(const size_type i,
                           const size_type j,
                           const Number    value)
{
  data[i][j] = value;
...
template <typename Number>
inline const typename MapMatrix<Number>::Iterator
...
template <typename Number>
inline void
MapMatrix<Number>::compress(const size_type row)
{
  typename Row::const_iterator       it     = data[row].begin();
  const typename Row::const_iterator it_end = data[row].end();
...
template <typename Number>
inline const typename MapMatrix<Number>::size_type *
...
template <typename Number>
inline typename MapMatrix<Number>::size_type
...
template <typename Number>
inline void
MapMatrix<Number>::print(std::ostream &out) const
{
  for (size_type i = 0; i < data.size(); ++i)
    {
      if (compressed_data[i].size() != 0)
...
//
template <typename Number>
inline MapMatrix<Number>::Accessor::Accessor(const Accessor &it)
  : map_it(it.map_it)
{}


template <typename Number>
inline MapMatrix<Number>::Accessor::Accessor(
  const typename Row::const_iterator &it)
  : map_it(it)
{}

template <typename Number>
inline const typename MapMatrix<Number>::Accessor &
...
//
template <typename Number>
inline MapMatrix<Number>::Iterator::Iterator(const Iterator &it)
  : accessor(it.accessor)
{}


template <typename Number>
inline MapMatrix<Number>::Iterator::Iterator(
  const typename Row::const_iterator &it)
  : accessor(it)
{}
...
template <typename Number>
inline bool
MapMatrix<Number>::Iterator::operator<(const Iterator &op) const
{
...
template <typename Number>
inline const typename MapMatrix<Number>::Accessor *MapMatrix<Number>::Iterator::
                                                   operator->() const
{
  return (&accessor);
}
